...
# ============================================================
#  DEVIATIONNET — apprend u, v + lambda, gamma, mu, zeta
# ============================================================

# Canaux des 2 premiers blocs : bloc0=1 canal, bloc1=2 canaux → 3 au total
...
class DeviationNet(nn.Module):
    def __init__(self, hidden=256, n_layers=6):
        ...
        out_features = 2 * N_CH_LEARNED * size * size + 4  # u+v sur blocs 0,1 + 4 scalaires
    ...
    def forward(self, x_bl, p_prev_bl, y_prev_bl, z_prev_bl, delta, n, T):
        ...
        for block in self.blocks:
            h = h + block(h)
        out = self.output_proj(h)   # (B, 2*N_CH_LEARNED*size*size + 4)

        # --- cartes u / v sur les 2 premiers blocs seulement ---
        maps = out[:, :-4].view(B, 2 * N_CH_LEARNED, size, size)

        # u : bloc0 (1 canal), bloc1 (2 canaux), blocs 2&3 → zéros
        u_raw = [
            torch.nan_to_num(maps[:, 0:1],              nan=0.0, posinf=0.0, neginf=0.0),  # bloc 0
            torch.nan_to_num(maps[:, 1:3],              nan=0.0, posinf=0.0, neginf=0.0),  # bloc 1
        ...
        v_raw = [
            torch.nan_to_num(maps[:, N_CH_LEARNED:N_CH_LEARNED+1], nan=0.0, posinf=0.0, neginf=0.0),  # bloc 0
            torch.nan_to_num(maps[:, N_CH_LEARNED+1:N_CH_LEARNED+3], nan=0.0, posinf=0.0, neginf=0.0), # bloc 1
            torch.zeros(B, SHAPES[2][1], size, size, device=flat.device),                               # bloc 2
            torch.zeros(B, SHAPES[3][1], size, size, device=flat.device),                               # bloc 3
        ]
        ...
        s   = out[:, -4:]
        eps = 1e-6
        lambda_n = float(torch.nn.functional.softplus(s[:, 0]).mean().item()) + eps
        gamma_n  = float(torch.nn.functional.softplus(s[:, 1]).mean().item()) + eps
        mu_n     = float(torch.nn.functional.softplus(s[:, 2]).mean().item()) + eps
        zeta_n   = float(torch.sigmoid(s[:, 3]).mean().item())
    ...
    def forward(self, noisy):
        ...
        for n in range(self.T):
            ...
            delta = compute_delta(
                ...
                a_n, th, th_h, th_b,gamma_n,lambda_n,gamma_prev,mu_n
            )
            delta = torch.nan_to_num(delta, nan=0.0, posinf=0.0, neginf=0.0)

            u_raw, v_raw, lambda_n, gamma_n, mu_n, zeta_n = self.dev_blocks(
            ...
            u_new, v_new = self.sg_layer(
                u_raw, v_raw, delta,
                lambda_n, mu_n, th, th_h, th_b, th_tilde, zeta_n
            )
