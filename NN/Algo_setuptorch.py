...
def get_setup(seed=0, noise_level=0.1):
    ...
    # noisy : (1, 64, 64)  →  on garde tel quel, Alg1 ajoutera la dim canal
    noisy_torch = torch.tensor(np.asarray(noisy), dtype=torch.float32).unsqueeze(0)

    return dict(
        space=space,
        noisy=noisy_torch,   # (1, 64, 64)
        K=K_torch,
    )
...
def build_algo_functions(setup, params):
    ...
    noisy_torch = setup['noisy']   # (1, 64, 64)
    K_torch     = setup['K']
    ...
    def C(u):
        # u[0] est (1, 1, H, W), noisy_torch est (1, H, W) → on aligne
