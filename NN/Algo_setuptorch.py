...
def get_setup(seed=0, noise_level=0.1, device=None):
    ...
    def odl_op(layer, x):
        """Run an ODL-backed layer on CPU, then move result back to x's device."""
        dev = x.device
    ...
    # noisy : (1, 64, 64) → envoyé directement sur le bon device
    ...
    return dict(
        space=space,
        noisy=noisy_torch,   # (1, 64, 64)  sur device
        K=K_torch,
        device=device,
    )
...
def build_algo_functions(setup, params):
    ...
    noisy_torch = setup['noisy']   # (1, 64, 64) sur device
    K_torch     = setup['K']
    device      = setup['device']
