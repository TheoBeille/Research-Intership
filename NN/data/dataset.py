...
def load_sample(size,seed, params, device, noise_range=(0.05, 0.2), clean_noise=0.0):
    """
    Build one sample:
    ...
    setup_noisy = get_setup(size,seed=seed, noise_level=noise_level, device=device)
    setup_clean = get_setup(size,seed=seed, noise_level=clean_noise, device=device)

    functions = build_algo_functions(setup_noisy, params)

    init_state = setup_noisy["initial_state"].unsqueeze(1).to(device)
    clean = setup_clean["initial_state"].unsqueeze(1).to(device)

    return init_state, clean, functions


def build_dataset(size,seeds, params, device, noise_range=(0.05, 0.2), clean_noise=0.0):
    """
