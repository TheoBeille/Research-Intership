...
def get_setup(size,seed=0, noise_level=0.1, device=None):
    ...
    T = odl.tomo.RayTransform(U, geometry,impl='astra_cuda')
    #T=odl.IdentityOperator(U)
    # NORMALIZATION
    ...
    x_true = torch.from_numpy(x_np).unsqueeze(0).unsqueeze(0).to(device)  # [1,1,H,W]

    noisy = A(x_true)           # → [1,1,angles,size]
...
def build_algo_functions(setup, params):
    ...
    def kkt_residual(x):
        ...
        data_grad = AT(A(u) - noisy)

        # K^T(Ku - y) + ∇^T p
        r1 = data_grad  + gradT_torch(p)
