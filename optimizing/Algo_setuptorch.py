...
def get_setup(seed=0, noise_level=0.1):

    space = odl.uniform_discr([0, 0], [100, 100], [64, 64])
    ...
    D_layer       = odl_torch.OperatorModule(D)          # D
    D_adj_layer   = odl_torch.OperatorModule(D.adjoint)  # D.T
    E_layer       = odl_torch.OperatorModule(E)          # E
    E_adj_layer   = odl_torch.OperatorModule(E.adjoint)  # E.T
    def K_torch(u):
        out0 = D_adj_layer(u[2])
        out1 = -u[2] + E_adj_layer(u[3])
        out2 = -D_layer(u[0]) + u[1]
        out3 = -E_layer(u[1])

        return [out0, out1, out2, out3]

    noisy_torch = torch.tensor(np.asarray(noisy), dtype=torch.float32).unsqueeze(0)
...
# ============================================================
#  Build All the functions needed of ALG1
# ============================================================
...
def build_algo_functions(setup, params):
    ...
    def C(u):
        return [u[0]-noisy_torch,torch.zeros_like(u[1]), torch.zeros_like(u[2]), torch.zeros_like(u[3])]
    ...
    def compute_delta_torch(p, x, p_prev, z, z_prev, y, y_prev, u, v, n):
        ...
        result = torch.clamp(term1 + term2 + term3, min=0.0)
    ...
    RA=RA_torch,
    C=C, compute_deviations=compute_deviations_torch
)
